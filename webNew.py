    ...
    if request.method == "POST":
        if 'file' in request.files:
            ...
            if fileExtension in Allowed_Image_Extenxion:
                ...
                # out = cv.VideoWriter('output6.mp4', fourcc, 30.0, (frame_width, frame_height))
                out = cv.VideoWriter(output_video_filename, fourcc, 30.0, (frame_width, frame_height))
                ...
                while cap.isOpened():
                    ...
                    # res_plotted = res_plotted.resize(frame_width, frame_height)
                    cv.namedWindow('result', cv.WND_PROP_FULLSCREEN)
                    cv.setWindowProperty('result', cv.WND_PROP_FULLSCREEN, cv.WINDOW_FULLSCREEN)
                    cv.imshow('result', res_plotted)
                    # cv.imshow('result', np.hstack((frame, res_plotted)))
...
def get_frame():
    ...
    # mp4files = 'output6.mp4'
    mp4files = output_video_filename 
